...
| 2 | `X-AiTrack-Timestamp` 与服务器时差 ≤ 300 秒 | 401 整批 | 防重放（12.2） |
| 3 | `X-AiTrack-Signature` HMAC 验证 | 401 整批 | 请求完整性 |
| 4 | 每条 `record_sig` HMAC 验证 | 单条 `rejected: sig_mismatch` | 防本地 DB 篡改（12.1/12.2） |
| 5 | `diff_hunk` 解析行数与 `added_lines`/`removed_lines` 偏差 ≤ 1 | 单条 `flagged: diff_inconsistent` | 防伪造 diff（12.4） |
| 6 | `repo_url` 在白名单内（enforce=true 时） | 单条 `flagged/rejected: repo_unknown` | 防 repo 伪造（12.9） |
| 7 | `file_path` 不含 `..`，与 `repo_url` 路径逻辑一致 | 单条 `flagged: path_mismatch` | 防路径注入（12.10） |
| 8 | `added_lines ≤ max_added_lines`（默认 5000） | 单条 `flagged: oversized` | 防行数膨胀（12.1/12.4） |
| 9 | (token_key, file_path) 每小时记录数 ≤ rate_limit（默认 30） | 单条 `rejected: rate_limited` | 防刷量（12.5） |
