...
**当前支持范围**：Claude Code、Codex CLI、Cursor 具备原生编辑适配器和原生提示词钩子，可生成带 diff、行数、仓库信息和签名的编辑证据，并补充有界提示词上下文。其他已登记工具主要通过稳定的本地会话、轨迹、导出、遥测和用量来源，以及心跳状态上报纳入可见范围。默认本地扫描覆盖 30 个已验证来源的规范工具 key。
