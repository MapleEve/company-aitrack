...
- **Rust client — `provider` field semantics**: `provider` now records the agent framework name directly (same as `tool` field: `claude`/`codex`/`cursor`); removed URL-based LLM backend inference; provider is consistent with tool field
- **UTC date bucketing consistency**: Rust (`chrono::Utc`), Java (`Instant.now()`), and Go (`time.Now().UTC()`) now all use UTC for daily bucketing, eliminating cross-timezone count drift
