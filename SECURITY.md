...
- **record_sig HMAC**：每条记录在写入本地 SQLite 前计算 HMAC-SHA256 签名，绑定 `token_key + device_id + hostname + timestamp + tool + file_path + repo_url + current_sha + added_lines + removed_lines + sha256(diff_hunk)`，防止本地记录被篡改后重传（hardening point 12.1/12.2）。
- **请求级签名**：上传请求携带 `X-AiTrack-Signature`（HMAC 覆盖时间戳和请求体哈希），服务端校验 300 秒时间窗口，防止重放攻击。
- **Myers/LCS 真差分**：使用 `similar` crate 计算最小差分，防止朴素行数统计被刷高（hardening point 12.4）。
- **心跳**：定期上报 hook 安装状态，服务端可发现 hook 被静默卸载（hardening point 12.3）。
- **解析失败记录日志**：适配器解析失败写 stderr，不静默吞错（hardening point 12.8）。
